...
const load = async (reset = false) => {
  ...
  try {
    ...
    const res = await listStarred(PAGE, offset)
    count.value = res.count
    requests.value = reset ? res.results : [...requests.value, ...res.results]
  } catch {
    error.value = 'Failed to load starred requests'
  } finally {
    loading.value = false
  }
}
  ...
  <div class="container mx-auto py-6">
    <div class="mb-6">
      ...
      <p class="text-sm text-muted-foreground mt-1">
        {{ count }} request{{ count === 1 ? '' : 's' }} you've starred.
      </p>
    </div>
    ...
    >
      No starred requests yet — tap the star on any request to save it here.
    </div>
