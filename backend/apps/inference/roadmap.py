...
PHASES = [
    {
        ...
        "tasks": [
            ...
            {"id": "scrape-node", "title": "`scrape` node kind + `scrape` manifest service type (Firecrawl via agent)", "status": STATUS_IN_PROGRESS, "note": "Vocabulary registered (SCRAPE/scrape, /v1/scrape, _resolve_kind, manifest_validator, builder). Firecrawl k8s manifest drafted in home-cluster (services/firecrawl, redis+playwright+api+worker, OPENAI_BASE_URL→cluster vLLM, type:scrape auto-discovered). Remaining: agent scrape handler (inference-club-agent repo) mapping {url}→Firecrawl POST /v1/scrape→OUTPUT_DOC."},
